window.onload = function () {
    ...
    core.storage.get('token',function (result) {
        token = result.token;
        core.post(api.getUrl('orderHandle'),{token:token,state:4},function (json) {
    ...
    function dataView(data) {
        ...
        for (var i = 0;i < len;++i) {
            ...
            html += '<td><input type="button" value="送件完成" class="tab-btn tab-small-btn" data-id="' +data[i].id+ '"><br>';
            html += '</td></tr>';
        }
        core.byTagName('tbody')[0].innerHTML = html;
        core.byClass('tab-small-btn').bindClick(function () {
            core.post(api.getUrl('done'),{token:token,id:this.dataset.id},function (json) {
                var jsonData = core.jsonParse(json);
